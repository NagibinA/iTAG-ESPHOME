...
|   -- | -- | -- |
|   <sub>Брелок с кнопкой, батарейку передает через нотификацию</sub>  |<sub>Брелок с кнопкой, батарейку не читает (заглушка 100%)</sub> | <sub>Брелок без кнопки, батарейку передает через чтение</sub> | 
  
<br/>

Полноценная прошивка для отслеживания BLE-устройств iTAG через ESP32 с поддержкой всех функций.

## Возможности

- 📍 **Отслеживание присутствия** - определение дома/не дома
- 🔘 **Кнопка** - мгновенная реакция на нажатие
- 🔋 **Батарея** - реальный уровень заряда 
...
### 1. Прошивка ESP32 для брелока с кнопкой, который батарейку передает через нотификацию
...
- Включите активный режим и подождите обновления (до 2 минут)
